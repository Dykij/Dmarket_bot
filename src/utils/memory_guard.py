...
class MemoryGuard:
    ...
    async def _check_memory(self, context: ContextTypes.DEFAULT_TYPE = None):
        ...
        try:
            ...
            if current_mb > self.limit_mb:
                logger.critical(f"MEMORY OVERFLOW: {current_mb:.1f}MB > {self.limit_mb}MB. REBOOTING...")
    ...
    def start(self, application: Application):
        ...
        logger.info(f"Starting MemoryGuard. Limit: {self.limit_mb}MB. Interval: {self.interval}s")
