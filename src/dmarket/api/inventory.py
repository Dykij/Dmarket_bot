...
class InventoryMixin:
    ...
    async def get_user_inventory(self, game_id: str = "a8db99ca-dc45-4c0e-9989-11ba71ed97a2", limit: int = 100) -> dict[str, Any]:
    ...
    async def list_user_offers(self, game_id: str = "a8db", status: str = "OfferStatusActive", limit: int = 100, offset: int = 0) -> dict[str, Any]:
        params = {"GameID": game_id, "Status": status, "Limit": str(limit), "Offset": str(offset)}
        return await self._request("GET", "/marketplace-api/v1/user-offers", params=params)

    async def create_offers(self, offers: list[dict[str, Any]]) -> dict[str, Any]:
        return await self._request("POST", "/marketplace-api/v1/user-offers/create", data={"Offers": offers})

    async def remove_offers(self, offer_ids: list[str]) -> dict[str, Any]:
        data = {"Offers": [{"OfferID": oid} for oid in offer_ids]}
        return await self._request("POST", "/marketplace-api/v1/user-offers/delete", data=data)
    ...
    async def sync_inventory(self, game_id: str = "a8db") -> dict[str, Any]:
        return await self._request("POST", "/marketplace-api/v1/user-inventory/sync", data={"GameID": game_id})
