...
class ExtendedMixin:
    ...
    async def get_sales_history(
        self, game: str, title: str, days: int = 7
    ) -> dict[str, Any]:
        params = {
            "gameId": game,
            "title": title,
    ...
    async def get_aggregated_prices_bulk(
        self, game: str, titles: list[str], limit: int = 100, cursor: str = ""
    ) -> dict[str, Any]:
        data = {
            "filter": {"game": game, "titles": titles},
            "limit": str(limit),
            "cursor": cursor,
        }
        return await self._request(
    ...
    async def get_market_best_offers(
        self, game: str = "csgo", title: str | None = None, limit: int = 50
    ) -> dict[str, Any]:
    ...
    async def get_active_offers(
        self, game: str = "csgo", limit: int = 50, offset: int = 0
    ) -> dict[str, Any]:
        params = {
            "gameId": game,
            "limit": limit,
    ...
    async def list_market_items_alt(
