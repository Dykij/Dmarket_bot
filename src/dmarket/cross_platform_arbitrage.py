...
class CrossPlatformArbitrageScanner:
    ...
    async def get_available_balance(self) -> Decimal:
        """Get available balance from DMarket.
    ...
    async def scan_full_market(
        ...
        if self.config.use_balance_limit:
            ...
            if balance <= 0:
                logger.warning("no_balance_available")
                return []
    ...
    async def _get_waxpeer_price(self, item_name: str) -> tuple[Decimal, int]:
        ...
        Uses WaxpeerPriceInfo for structured data if available.
        ...
        try:
            ...
            # Try to use get_item_price_info if available
