...
class LiquidityAdjustedFee:
    ...
    # Default risk-free daily return for opportunity cost (e.g., Grind Strategy ROI)
    # If we can make 1% daily by grinding, locking funds for 7 days costs us ~7.2%
    DEFAULT_DAILY_ROI = Decimal("0.01") 

    def __init__(
        self, 
        dmarket_fee_rate: Decimal = DM_FEE_BOT_MAX, 
        daily_opportunity_cost: Decimal = DEFAULT_DAILY_ROI
    ):
        """
        ...
        Args:
            ...
            daily_opportunity_cost: Expected daily return on capital if not locked (0.01 = 1%).
        """
        self.dm_fee_rate = dmarket_fee_rate
        self.daily_opportunity_cost = daily_opportunity_cost

    def calculate_opportunity_cost(self, capital: Decimal, days: int = STEAM_HOLD_DAYS) -> Decimal:
        """
        ...
        compound_factor = (Decimal("1") + self.daily_opportunity_cost) ** days
        future_value = capital * compound_factor
        return (future_value - capital).quantize(Decimal("0.01"), rounding=ROUND_HALF_UP)
    ...
    def get_break_even_waxpeer(self, buy_price: float | Decimal) -> Decimal:
        ...
        effective_multiplier = (Decimal("1") - self.WAX_SELLING_FEE) * (Decimal("1") - self.WAX_CASHOUT_FEE)
