...
async def cmd_test_trade(message: types.Message):
    ...
    try:
        ...
        if cs_price <= 0:
            await message.answer(f"❌ CS2Cap Oracle failed to find reliable Reference Price for `{item_name}`.")
            return

        # Mathematical Validation (DMarket -> CS2Cap resale logic limit)
        try:
            # Check if buying at DM and selling at CS2Cap is feasible based on our 5% margin
            ...
            await message.answer(
                ...
                f"📈 CS2Cap Oracle (BUFF163): `${cs_price:.2f}`\n"
