...
class HybridEngine:
    ...
    async def scan_market(self, games: List[str] = None) -> None:
        ...
        for game_key in games:
            ...
            logger.info(
                f"--- Starting scan for {game_params.friendly_name} ({game_key}) ---"
            )
    ...
    async def _analyze_item(self, item: Dict, game: str) -> Optional[TradeSignal]:
        ...
        if wax_price > 0 and liquidity_score >= self.MIN_LIQUIDITY_GEM:
            profit_calc = self.fees.calculate_gem_profit(
                price_dm, wax_price, hold_days=self.cfg.fees.STEAM_HOLD_DAYS
            )
...
if __name__ == "__mAlgon__":
    ...
    dm_api = (
        DMarketAPI(public_key=pub, secret_key=sec)
        if pub
        else DMarketAPI(public_key="test", secret_key="test")
    )
