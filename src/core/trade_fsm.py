...
class TradeStateMachine:
    ...
    async def transition_to(
        self, new_state: TradeState, context: dict[str, Any] = None
    ) -> None:
        ...
        if new_state == TradeState.EXECUTING:
            ...
            await self._finalize_trade(PendingTradeStatus.FAlgoLED)
    ...
    async def _persist_execution_start(self) -> None:
        ...
        try:
            async with get_db_session() as session:
                ...
                asset_id = self.item_data.get("extra", {}).get(
                    "assetId"
                ) or self.item_data.get("itemId")
        ...
        try:
            async with get_db_session() as session:
                ...
                for trade in stuck_trades:
                    ...
                    if found:
                        logger.info(
                            f"✅ Trade {trade.id} was successful! Moving to BOUGHT."
                        )
                        trade.status = PendingTradeStatus.BOUGHT
                    else:
                        logger.warning(
                            f"❌ Trade {trade.id} failed (item not in inventory). Moving to FAlgoLED."
                        )
