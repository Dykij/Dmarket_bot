...
async def system_restart_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
...
def register_system_handlers(application):
    ...
    application.add_handler(CallbackQueryHandler(system_status_command, pattern="^system_status$"))
    application.add_handler(CallbackQueryHandler(system_restart_callback, pattern="^system_restart$"))
