...
from src.telegram_bot.handlers.keyboard_parts.ml_Algo import (
    ml_Algo_menu_callback,
    ml_Algo_status_callback,
)
...
def get_main_keyboard(balance: float | None = None) -> InlineKeyboardMarkup:
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🧠 ML/Algo ОБУЧЕНИЕ", callback_data="ml_Algo_menu")],
        [
...
def register_main_keyboard_handlers(application) -> None:
    ...
    application.add_handler(
        CallbackQueryHandler(ml_Algo_menu_callback, pattern="^ml_Algo_menu$")
    )
    application.add_handler(
        CallbackQueryHandler(ml_Algo_status_callback, pattern="^ml_Algo_status$")
    )
    application.add_handler(
