...
async def main_menu_callback(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.answer()
    await query.edit_message_text("👋 <b>Главное меню</b>", parse_mode=ParseMode.HTML, reply_markup=get_main_keyboard())

async def emergency_stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    query = update.callback_query
    await query.answer("⚠️ ЭКСТРЕННАЯ ОСТАНОВКА!")
    await query.edit_message_text("🛑 Все процессы остановлены.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Главное меню", callback_data="main_menu")]]))
