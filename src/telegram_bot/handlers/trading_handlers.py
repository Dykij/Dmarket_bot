...
async def auto_trade_start(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    if is_running:
        ...
        await query.edit_message_text("🤖 🟢 <b>Статус: РАБОТАЕТ</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(keyboard))
    else:
        keyboard = [
            [InlineKeyboardButton("🚀 ЗАПУСТИТЬ", callback_data="auto_trade_run")],
            [InlineKeyboardButton("🔎 СКАНИРОВАТЬ ВСЁ", callback_data="auto_trade_scan_all")],
            [InlineKeyboardButton("◀️ Главное меню", callback_data="main_menu")],
        ]
        await query.edit_message_text("🤖 🔴 <b>Статус: ОСТАНОВЛЕНА</b>", parse_mode="HTML", reply_markup=InlineKeyboardMarkup(keyboard))
...
async def auto_trade_run(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text("✅ Авто-торговля запущена!", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="auto_trade_start")]]))
...
async def auto_trade_stop(update: Update, context: ContextTypes.DEFAULT_TYPE) -> None:
    ...
    await query.edit_message_text("🛑 Торговля остановлена.", reply_markup=InlineKeyboardMarkup([[InlineKeyboardButton("◀️ Назад", callback_data="auto_trade_start")]]))
