...
class DMarketAPIClient:
    ...
    def __init__(self, public_key: str, secret_key: str, base_url: str = "https://api.dmarket.com"):
        ...
        try:
            ...
            self._rust_signer = dmarket_parser_rs.generate_signature_rs
            logger.info("High-performance Rust signer active (dmarket_parser_rs).")
        except ImportError:
            try:
                import rust_core
                self._has_rust_signer = True
                self._rust_signer = rust_core.generate_signature_rs
                logger.info("High-performance Rust signer active (rust_core).")
            except ImportError:
    ...
    async def create_sell_offer(self, game_id: str, item_id: str, price_usd: float) -> Dict[str, Any]:
        """
        List an item for sale on DMarket.
        item_id: The inventory item ID to sell.
        price_usd: Sell price in USD (will be converted to cents internally).
        """
    ...
    async def create_sell_offers_batch(self, game_id: str, items: List[Dict[str, Any]]) -> Dict[str, Any]:
        """
        Batch create sell offers.
        items: [{"item_id": "...", "price_usd": 10.50}, ...]
        """
        offer_items = []
        for item in items:
            price_cents = int(item["price_usd"] * 100)
            offer_items.append({
                "gameId": game_id,
                "itemId": item["item_id"],
                "price": {"amount": str(price_cents), "currency": "USD"}
            })
        return await self.make_request("POST", "/marketplace-api/v1/user-offers/create", body={"items": offer_items})

    async def edit_sell_offer(self, offer_id: str, new_price_usd: float) -> Dict[str, Any]:
        """Edit an existing sell offer's price."""
        price_cents = int(new_price_usd * 100)
        body = {
            "offerId": offer_id,
            "price": {"amount": str(price_cents), "currency": "USD"}
        }
    ...
    async def delete_sell_offer(self, offer_id: str) -> Dict[str, Any]:
        """Delete/cancel a sell offer."""
        return await self.make_request("DELETE", "/exchange/v1/offers", params={"offerId": offer_id})
    ...
    async def get_item_fee(self, game_id: str, item_id: str, price_cents: int) -> float:
        ...
        In-memory cache (12h) + SQLite persistent cache + DRY_RUN fast path.
        """
        # DRY_RUN: skip API call entirely, use default fee
        if os.getenv("DRY_RUN", "true").lower() == "true":
            return 0.05
        ...
        cached_fee = price_db.get_state(f"fee_{item_id}")
        if cached_fee is not None:
            try:
                fee_data = __import__("json").loads(cached_fee)
                if now - fee_data["ts"] < self._fee_cache_ttl:
                    self._fee_cache[item_id] = {"fee": fee_data["fee"], "timestamp": fee_data["ts"]}
                    return fee_data["fee"]
            except Exception:
                pass
        ...
        try:
            ...
            import json
            price_db.save_state(f"fee_{item_id}", json.dumps({"fee": fee_pct, "ts": now}))
            return fee_pct
        except Exception as e:
