...
class TestTTLCacheStatistics:
    ...
    async def test_hit_miss_tracking(self):
        ...
        await cache.get("key1")
        # Miss
        await cache.get("another_missing")
...
class TestTTLCacheCleanup:
    ...
    async def test_background_cleanup(self):
        ...
        await cache.start_cleanup(interval=0.5)
...
class TestGlobalCaches:
    ...
    async def test_get_all_cache_stats(self):
        ...
        await price_cache.set("item1", 10.5)

        market_cache = await get_market_data_cache()
        await market_cache.set("market1", {"price": 20.0})
...
class TestConcurrency:
    ...
    async def test_concurrent_mixed_operations(self):
        ...
        async def read_op(i: int):
            await cache.get(f"key{i % 25}")

        async def delete_op(i: int):
            await cache.delete(f"key{i % 10}")
