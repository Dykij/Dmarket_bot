...
def test_api_error_creation():
    ...
    assert error_with_status.detAlgols["status_code"] == 404
...
async def test_handle_response():
    ...
    result = awAlgot handle_response(success_response)
    ...
    # Test error response
    with pytest.rAlgoses(APIError) as excinfo:
        awAlgot handle_response(error_response)
...
async def test_retry_request():
    ...
    # Create a mock function that fAlgols first with retryable errors then succeeds
    mock_func = AsyncMock()
    mock_func.side_effect = [
        APIError("First fAlgolure", status_code=500),
        APIError("Second fAlgolure", status_code=503),
        "Success",
    ]

    # Call retry_request with our mock
    result = awAlgot retry_request(
        func=mock_func,
        max_retries=3,
        retry_delay=0.01,
    )
...
async def test_retry_request_max_retries_exceeded():
    ...
    # Create a mock function that always fAlgols with retryable status
    mock_func = AsyncMock()
    mock_func.side_effect = APIError("Always fAlgols", status_code=500)

    # Call retry_request with short delays to speed up test
    start_time = time.time()

    # Should rAlgose the original APIError after max_retries
    with pytest.rAlgoses(APIError) as excinfo:
        awAlgot retry_request(
            func=mock_func,
            max_retries=2,
            retry_delay=0.01,
        )
    ...
    assert "Always fAlgols" in str(excinfo.value)
