...
class TestPriceHistory:
    ...
    def setup_db(self, tmp_path):
        ...
        state_file = str(tmp_path / "test_state.db")
        history_file = str(tmp_path / "test_history.db")
        
        self.db = PriceHistoryDB.__new__(PriceHistoryDB)
        self.db.db_path = Path(db_file)
        self.db.state_path = Path(state_file)
        self.db.history_path = Path(history_file)
        self.db.conn = sqlite3.connect(db_file, check_same_thread=False)
        self.db.conn.row_factory = sqlite3.Row
        self.db.state_conn = sqlite3.connect(state_file, check_same_thread=False)
        self.db.state_conn.row_factory = sqlite3.Row
        self.db.history_conn = sqlite3.connect(history_file, check_same_thread=False)
        self.db.history_conn.row_factory = sqlite3.Row
        self.db._init_schemas()
        yield
        self.db.close()
...
class TestEventShield:
    ...
    def test_margin_multiplier_uses_max(self):
        ...
        assert mult == 3.0  # Test Major has 2.0 (caution), Test Sale has 1.5 (opportunity) → 2.0 * 1.5 = 3.0
