...
class TestIsDemandOpportunity:
    ...
    def test_basic_batch(self):
        ...
        opps = is_demand_opportunity(agg_data, max_price=10.0, min_price=0.50)
        # Item A: Q=3.0 > 2.0, should pass
        # Item B: Q=1.0 < 2.0, should fail
        assert len(opps) == 1
        assert opps[0]["title"] == "Item A"
    ...
    def test_price_filter(self):
        ...
        assert len(opps) == 0  # Above max_price

    def test_empty_data(self):
        assert is_demand_opportunity({}) == []
...
class TestDynamicStopLoss:
    """Test dynamic stop-loss parameters."""
    ...
    def test_adaptive_hold_time(self):
        ...
        t_cheap = get_adaptive_thresholds(1.00)
        assert t_cheap["max_hold_days"] == 10.0

        # Expensive items: shorter hold
        t_expensive = get_adaptive_thresholds(8.00)
        assert t_expensive["max_hold_days"] == 5.0


class TestPeakAvoidance:
    """Test peak avoidance logic."""

    def test_obi_values_in_range(self):
        result = calculate_demand_score(
            title="Test",
            ask_price=5.00,
            best_bid=4.80,
        ...
        assert -1.0 <= result["obi_value"] <= 1.0

    def test_micro_price_reasonable(self):
        result = calculate_demand_score(
            title="Test",
            ask_price=5.00,
            best_bid=4.80,
        ...
        assert 4.50 <= result["micro_price"] <= 5.50
