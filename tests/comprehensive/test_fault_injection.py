...
class TestDependencyFailure:
    ...
    @pytest.mark.asyncio
    async def test_cache_unavailable_fallback(self) -> None:
    ...
    @pytest.mark.asyncio
    async def test_external_api_unavailable(self) -> None:
...
class TestCascadingFailures:
    ...
    async def test_graceful_degradation_under_load(self) -> None:
        ...
        available = get_features_for_load(load_level)

        assert "core" in available
        assert "analytics" not in available  # Degraded
        assert "recommendations" not in available  # Degraded
