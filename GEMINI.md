...
- **Message Regeneration Integrity:** Если хук/система блокирует отправку сообщения,
  НЕЛЬЗЯ отбрасывать или пересказывать своими словами уже подготовленный RAW-контент
  этого сообщения. Нужно закрыть ТОЛЬКО конкретную причину блокировки (например,
  дозаполнить task.md) и отправить ИСХОДНЫЙ контент как есть, добавив при необходимости
  краткое объяснение блокировки — не вместо данных, а вместе с ними.
  Если исходный текст сообщения физически недоступен для восстановления —
  ОБЯЗАН заново выполнить команды, а не извиняться и продолжать без данных.
  Для RAW-вывода команд, если есть риск потери при пересборке сообщения —
  цитировать из .agents/logs/RAW_OUTPUT.log (туда пишет raw-output-logger
  автоматически), а не восстанавливать по памяти.
