...
run_checks() {
    ...
    TERMINATION_REASON=$(echo "$INPUT" | jq -r '.terminationReason // empty')
    FULLY_IDLE=$(echo "$INPUT" | jq -r '.fullyIdle // empty')
    EXECUTION_NUM=$(echo "$INPUT" | jq -r '.executionNum // empty')

    if [ "$TERMINATION_REASON" = "model_stop" ] && [ "$FULLY_IDLE" = "true" ]; then
        cd "$WORKSPACE_DIR"
        UNTRACKED=$(git status --porcelain 2>/dev/null | grep "^??" | cut -d' ' -f2)
        SCRATCH_FILES=""
        for f in $UNTRACKED; do
            if [[ "$f" == tmp_* ]] || [[ "$f" == *_results.txt ]] || [[ "$f" == *_results*.txt ]] || \
               [[ "$f" == test_debug* ]] || [[ "$f" == cst_*.py ]] || [[ "$f" == explore_*.sh ]] || \
               [[ "$f" == gap_closure* ]] || [[ "$f" == *_full.log ]] || [[ "$f" == baseline_*.log ]] || \
               [[ "$f" == diff_full.txt ]]; then
                SCRATCH_FILES="$SCRATCH_FILES $f"
            fi
        done
        
        if [ -n "$SCRATCH_FILES" ]; then
            SCRATCH_COUNT_FILE=".agents/scratch/.stop_scratch_guard_count"
            mkdir -p .agents/scratch
            S_COUNT=$(cat "$SCRATCH_COUNT_FILE" 2>/dev/null || echo 0)
            S_COUNT=$((S_COUNT + 1))
            if [ "$S_COUNT" -lt 4 ]; then
                echo "$S_COUNT" > "$SCRATCH_COUNT_FILE"
                jq -n --arg reason "Обнаружены scratch-файлы: $SCRATCH_FILES. Удалите их перед завершением." '{"decision":"continue","reason":$reason}'
                return
            fi
            echo "$S_COUNT" > "$SCRATCH_COUNT_FILE"
        fi
