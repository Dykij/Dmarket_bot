...
- Skill file: `.mimocode/skills/deep-code-review/SKILL.md`
...
1. **Never push directly to `origin`** — raw `git push` is BLOCKED by `mimocode.jsonc` permission system
...
- `.mimocode/skills/git-gate/SKILL.md` — Gate definition
- `.mimocode/skills/skill-workflow-integrator/SKILL.md` — Workflow orchestrator
- `.mimocode/mimocode.jsonc` — Permission config (blocks raw `git push`)
- `.mimocode/skills/git-gate/scripts/git-gate.sh` — Standalone gate script

---

## Архитектура торговли — ВАЖНО

Этот бот НЕ ведёт кросс-платформенную торговлю. Единственная площадка,
...
## Ограничения окружения MimoCode

**Важно:** MimoCode (v0.1.7) — нативный бинарник, а не Node.js приложение. Следующие 6 плагинов оркестрации **НЕ совместимы** с MimoCode и **НЕ загружаются** его рантаймом:

| Плагин | Версия | Статус | Причина |
|--------|--------|--------|---------|
| opencode-conductor | 1.0.6 | ❌ Не загружается | Написан под Node.js runtime OpenCode |
| opencode-background-agents | 0.1.1 | ❌ Не загружается | Написан под Node.js runtime OpenCode |
| opencode-scheduler | 1.3.0 | ❌ Не загружается | Написан под Node.js runtime OpenCode |
| opencode-goal-plugin | 0.6.5 | ❌ Не загружается | Написан под Node.js runtime OpenCode |
| @bluelovers/opencode-arise | 0.1.40 | ❌ Не загружается | Написан под Node.js runtime OpenCode |
| opencode-handoff | 0.5.0 | ❌ Не загружается | Написан под Node.js runtime OpenCode |

**Причина:** Плагины зависят от `@opencode-ai/plugin` (OpenCode API). MimoCode предоставляет `@mimo-ai/plugin` (собственный API). Shim/алиас между ними не существует. Команда `mimo plugin <name>` устанавливает пакет в кеш, но рантайм не сканирует эту директорию.

**Не пытаться использовать эти плагины.** Не тратить время на повторные попытки — проблема архитектурная, не решается локальными патчами.

**Альтернативы (работают в MimoCode):**
- Параллельные суб-агенты → встроенный `actor` tool (subagent_type: explore/general/analyst/security/trader)
- Расписания → встроенный `cron` tool (schedule/loop)
- Навыки → SKILL.md файлы в `.mimocode/skills/` и `.opencode/skills/`
- Оркестрация → Compose mode (compose:plan, compose:execute, compose:verify)

---

## MCP-серверы: статус и назначение

| Сервер | Статус | Назначение |
|--------|--------|------------|
| sequential-thinking | ✅ Работает | Многошаговое рассуждение, планирование |
| archy | ✅ Работает | Проверка архитектуры (modularity, cycles, complexity) |
| sqlite | ✅ Работает | Запросы к SQLite БД (price history, trades) |
| context7 | ✅ Работает | Актуальная документация библиотек (aiohttp, Pydantic, SQLAlchemy) |
| fetch | ✅ Работает | HTTP-запросы, получение веб-страниц |
| web-search | ✅ Работает | Веб-поиск (DuckDuckGo, Bing, SearXNG) |
| shellcheck | ✅ Работает | Линтинг bash/shell скриптов |
| in-memoria | ✅ Работает | Обучение паттернам кодовой базы, семантический поиск |
| semgrep | ✅ Работает | Security scanning (обновлён на `semgrep mcp`, старый `uvx semgrep-mcp` deprecated) |
| filesystem | 🔒 Отключён | Дублирует встроенные инструменты MimoCode |
| git | 🔒 Отключён | Дублирует встроенные инструменты MimoCode |
| memory | 🔒 Отключён | Дублирует in-memoria |
| playwright | 🔒 Отключён | Не нужен — бот работает через REST API, не через браузер |
| youtube-transcript | 🗑️ Удалён | Не используется проектом, удалён из конфига |

---

## Статус проекта после аудита

**Дата аудита:** 2026-07-22 | **Прогонов:** 3 | **Файлов проанализировано:** 30+

**Исправлено:** 13 файлов, 13 исправлений (P1×9, P2×4)
**P0 проблем:** 0
...
## 🔧 MCP/LSP — Mandatory Usage

**When working on ANY request or generating code, ALWAYS use the full toolchain:**

### MCP Servers (9 active, configured in `.mimocode/mimocode.jsonc`)
...
| `semgrep` | Security scanning — ALWAYS scan generated code |
| `shellcheck` | Shell script linting |
| `in-memoria` | Codebase pattern learning, semantic search |
