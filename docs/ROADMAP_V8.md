...
## Phase 8: Laboratory of Future (The Experiment)
*   `[SYSTEM_CORE]` **MCP Native Server:** [ ] Direct integration of Model Context Protocol to expose local Windows filesystem safely to N8N workflows.
*   `[SYSTEM_CORE]` **Async Event Bus:** [x] Replacing direct function calls between agents with a Redis/ZeroMQ message bus for non-blocking swarm hive-mind.
*   `[SYSTEM_CORE]` **Tree-sitter Code Indexing:** [ ] Advanced code parsing to understand the *structure* of the bot (classes, functions) rather than just text matching.
*   `[SYSTEM_CORE]` **Local Inference Fallback:** [x] Integration with Ollama to spin up a local small model (e.g., DeepSeek Coder) if Cloud API goes down.
*   `[SYSTEM_CORE]` **Self-Healing AST Rewriter:** [x] Using Tree-sitter to automatically refactor deprecated function calls across the codebase without regex.
