...
Every review finding MUST pass these checks before reporting:
1. **Source**: Have I read the actual file? (not from memory)
2. **Citation**: Do I have file:line with direct code quote?
3. **Verification**: Can I reproduce the issue?
4. **Confidence**: Is my confidence calibrated? (>90% only with direct evidence)
5. **Hallucination type**: Am I at risk of any of the 8 hallucination types?

**If ANY check fails → Do NOT report the finding. Verify first or withdraw.**

### Chain-of-Verification (CoVe) for Important Findings

Before reporting an IMPORTANT/CRITICAL finding:
```
STEP 1: State the finding
STEP 2: List 3 factual claims in my finding
STEP 3: Verify each against actual code (YES/NO with evidence)
STEP 4: Revise — remove unverified claims
```

### Confidence Calibration for Findings

| Finding Type | Required Confidence | Required Evidence |
|-------------|--------------------|--------------------|
| CRITICAL/IMPORTANT | ≥ 90% | Direct file:line quote + reproduction |
| WARNING | ≥ 70% | Direct file:line quote |
| NIT | ≥ 50% | File reference |
