...
6. **Явное указание файла ("@"-нотация)**:
   - Если в сообщении пользователя присутствует `@<путь-к-файлу>` — это явный, однозначный указатель на целевой файл правки. Агент обязан работать именно с этим файлом, не подменяя его результатом собственного поиска/сканирования и не расширяя изменение на другие файлы без отдельного упоминания.
   - Если упомянутый через `@` файл не существует по указанному пути — агент обязан остановиться и явно сообщить об этом, а не угадывать похожий файл.
   - Эта нотация не отменяет Scope discipline из GEMINI.md/documentation-grounding.md: правки за пределами файла, указанного через `@`, по-прежнему требуют отдельного явного разрешения, даже если они кажутся логически связанными.
